...
class HorsesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for url in urls:
            yield scrapy.Request(url=url, meta={'item': item, 'dont_redirect': True,
                  'handle_httpstatus_list': [301, 302, 403]},  callback=self.parse)
    ...
    def parse(self, response):
        ...
        item['odds'] = odds or [0] *21
        item['url'] = response.url or [''] * 21
        yield item
